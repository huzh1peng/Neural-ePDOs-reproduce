...
class pdo_net(nn.Module):
    def __init__(self, n, flip, dis='fd', channel=[16, 24, 32, 32, 48, 64], order=4, g=4, reduction_ratio=1, s=4,
                 drop=0.1, type='regular'):
        ...
        # Define initial convolutional and normalization layers
        ...
        if g == -1:
            ...
            self.conv2 = self.group.nlpdo((self.type, channel[0]), (self.type, channel[1]), order,
                                          reduction=reduction_ratio, s=s, g=g)
            self.conv3 = self.group.nlpdo((self.type, channel[1]), (self.type, channel[2]), order,
                                          reduction=reduction_ratio, s=s, g=g)
            self.conv4 = self.group.nlpdo((self.type, channel[2]), (self.type, channel[3]), order,
                                          reduction=reduction_ratio, s=s, g=g)
            self.conv5 = self.group.nlpdo((self.type, channel[3]), (self.type, channel[4]), order,
                                          reduction=reduction_ratio, s=s, g=g)
            if flip:
                self.group = Group(n, False, dis)
                ...
                self.channel[4] *= 2
                self.channel[5] *= 2
            self.conv6 = self.group.nlpdo((self.type, channel[4]), (self.type, channel[5]), order,
                                          reduction=reduction_ratio, s=s, g=g)
